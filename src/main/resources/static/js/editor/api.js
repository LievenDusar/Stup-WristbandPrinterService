// Thin wrappers over the /api/wristband-templates and /api/wristband-assets endpoints. The admin cookie rides along
...
export async function listTemplates() {
  const res = guard(await fetch('/api/wristband-templates'));
  return res.ok ? res.json() : [];
}

export async function getTemplate(id) {
  const res = guard(await fetch('/api/wristband-templates/' + id));
...
export async function uploadAsset(file) {
  ...
  fd.append('file', file);
  const res = guard(await fetch('/api/wristband-assets', { method: 'POST', body: fd }));
